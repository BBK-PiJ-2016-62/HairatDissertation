import BikeSharingLnrRegrssnOneMnth._
...
object BikeSharingHourPredictionAndEvaluation_3rdYrMnthHrWDWthr {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val data00 = dataRDDNoHeader.map(line => {
      val arr = line.split(",")
      (arr(4).toDouble, arr(3).toDouble, arr(5).toDouble, arr(8).toDouble, rerangeWthrAccordToCnt(arr(9)), arr(10).toDouble,
    ...
    val data0 = data00.map(row=>(row._1, row._2, rerangeHrsAccordToCnt1(row._3)(sortHrList), row._4, rerangewthrAccordToCnt1(row._5)(sortWthrList),
      rerangeAtempAccordToCnt1(row._7)(sortAtempList), rerangeHumAccordToCnt1(row._8)(sortHumList),
    ...
    var vectIdxr = new VectorIndexer().
      ...
      setOutputCol("features").setMaxCategories(13)
