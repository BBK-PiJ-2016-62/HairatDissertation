import BikeSharing_5th_83_DcsTrRgrssnOneMnth._
...
object BikeSharingDcsTrRgrssnWhlYrPeparaErly {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val set1 = data000.randomSplit(Array(0.8, 0.2), seed = 700)
    val data00 = set1(0)
    val data01 = set1(1)
    ...
    val sortATempList = sortTemp(data00.map(row => (row._6, row._9)))
    ...
    val data0 = data00.map(row=>(rerangeMnthAccordToCnt1(row._1)(sortMnthList), row._2,
      ...
      rerangeTempAccordToCnt1(row._6)(sortATempList),
      rerangeHumAccordToCnt1(row._7)(sortHumList),
      rerangeWndSpAccordToCnt1(row._8)(sortWindSpList), row._9))
    val dataTt0 = data00.map(row=>(rerangeMnthAccordToCnt1(row._1)(sortMnthList), row._2,
      rerangeHrsAccordToCnt1(row._3)(sortHrList), row._4,
      rerangewthrAccordToCnt1(row._5)(sortWthrList),
      rerangeTempAccordToCnt1(row._6)(sortATempList),
      rerangeHumAccordToCnt1(row._7)(sortHumList),
